...
* Save data?
* Handle tab closing and re-execution
* Check Internet Connection
* Add follower profile photos
* Jump to page
* Status messages and icons with AngularJS
*/
...
(function () {
  ...
  function parseProfile(profileID, profileAboutPage) {
    var reProfilePhoto = /<meta itemprop="image" content="([^"]*)photo.jpg">/,
        reProfileFollowers = /^,\[(\d+),\[\[\"|^,\[0,\[\]/gm,
        result,
        profilePhoto = '',
        profileFollowers = -1;
    if (result = reProfilePhoto.exec(profileAboutPage)) {
      if (result[1]) {
        profilePhoto = 'http:' + result[1] + 's64/photo.jpg';
      }
    }
